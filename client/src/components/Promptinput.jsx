...
const Promptinput = ({onSubmit, loading = false, placeholder = "Describe the website you want to build...", large = false, autoFocus = false, varient = "default"}) => {
    ...
    const handleKeyDown = (e)=>{
       ...
       if(varient === "glass"){
        return(
            <form onSubmit={handleSubmit} className='max-w-2xl w-full bg-white/10 backdrop-blur-xl rounded-xl ring-1 ring-white/25 focus-within:ring-2'>
                ...
                <textarea ref={textareaRef} value={value} onChange={(e)=>setValue(e.target.value)} onKeyDown={handleKeyDown} placeholder={placeholder} disabled={loading} rows={3} className='w-full p-4 pb-2 resize-none placeholder:text-white/60 outline-none bg-transparent text-white text-base'/>
                    <div className='flex items-center justify-between pb-3 px-3 gap-2'>
                        <label htmlFor="file" className='border border-white/20 text-white/80 hover:text-white hover:border-white/30 rounded-md cursor-pointer flex items-center justify-center'>
                        ...
                        <div className='flex items-center justify-end gap-2'>
                            <button type='button' className='flex items-center justify-center p-1 text-white/70 hover:text-white cursor-pointer'>
                             ...
                             className='flex items-center justify-center p-1 rounded-full bg-red-600 text-white hover:bg-red-700 disabled:opacity-40 cursor-pointer '>
  ...
  return (
    <div className={`bg-white border border-zinc-200 rounded-xl flex items-end gap-2 focus-within:ring-1 focus-within:ring-zinc-300 transition ${large ? "p-4" : "p-3"}`}>
        ...
        disabled={loading} 
        rows={large ? 5 : 1} 
        className={`flex bg-transparent border-none outline-none resize-none text-zinc-900 placeholder:text-zinc-400 ${large ? "text-base" : "text-sm"}`}/>
